...
fn file_and_subfiles_with_corresponding_modules<'db>(
    ...
    // Item-level macros generate modules as the `ModuleId::MacroCall` variant. Unlike regular
    // submodules, these are not reachable via `db.file_modules`, so they are missing from
    // `modules`. We still need the files they generate: both to expose the macro-generated code
    // to callers and because those files can host further inline macro calls. For every module we
    // already have, `module_macro_modules` returns it followed by the whole chain of macro-call
    // submodules, so this descends into all of them.
    let all_modules: OrderedHashSet<ModuleId<'db>> = modules
        .iter()
        .flat_map(|&module_id| module_macro_modules(db, true, module_id).iter().copied())
        .collect();

    // Add the files generated by macro-call modules. Regular modules' files are already in
    // `files`, so this effectively adds the macro-generated ones. They must be present before the
    // inline expansion walk below, which stops once it reaches a file already in `files`.
    for &module_id in all_modules.iter() {
        if let Ok(module_files) = db.module_files(module_id) {
            files.extend(module_files.iter().copied());
        }
    }
